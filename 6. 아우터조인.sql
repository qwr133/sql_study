...
*
FROM tb_emp A --왼쪽 테이블
inner JOIN tb_dept B --OUTER 생략가능
...
*
FROM tb_emp A --왼쪽 테이블
LEFT outer JOIN tb_dept B --OUTER 생략가능. 데이터가 매치되어 있지 않기때문에 null값으로 나옴
ON A.dept_cd = B.dept_cd
ORDER BY A.emp_no 
;
SELECT 
...
WHERE A.dept_cd = B.dept_cd (+)  --+ -> 레프트조인? 
ORDER BY A.emp_no
;
...
SELECT 
    A.emp_no, A.emp_nm, B.dept_cd, B.dept_nm
FROM tb_emp A, tb_dept B
WHERE A.dept_cd (+) = B.dept_cd --where에서 + 일반조건이 아니라 조인조건
ORDER BY A.emp_no 
;

-- #FULL OUTER JOIN - 왼쪽 오른쪽 다 보여줘 null값도 포함해서~~ 41개, 5명 null 부서, 2부서 총 48개 --> 오라클에 없음, 안시 표준에만 있음
