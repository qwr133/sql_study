...
--문자열은 유니코드값으로 계산하기 때문에 TRUE 가 나옴
SELECT 
    REPLACE(char_4, ' ', '+') AS char_4,
    REPLACE(char_6, ' ', '+') AS char_6
FROM char_compare
...
FROM varchar_compare
WHERE sn = '101'
    AND char_4 = TRIM(varchar_6) --trim 앞 뒤 공백 지우기
;
...
SELECT 
    ...
    SUBSTR('SQL Developer', 1, 3), -- 문자열 자름 1번부터 3개자름 (첫글자가 1번(SQL만))
...
SELECT 
    SYSDATE AS "현재 시간",
    SYSDATE - 1 AS "SYSDATE - 1", -- 오늘날짜에서 하루 전날
...
-- 문자를 숫자로 변환
SELECT 
    TO_NUMBER('$5,500', '$999,999') - 4000 AS "계산결과" --$999,999 정수값으로 바꾼 후 -4000해야 계산이 가능함
FROM dual;


-- CASE표현과 DECODE함수
SELECT * FROM tb_sal;
-- Searched expression 대소비교 가능
SELECT 
    sal_cd, 
    sal_nm,
...
-- Simple expression -- 일치비교만 가능 (대소비교 안됨)
SELECT 
    sal_cd, 
    sal_nm,
    CASE sal_cd 
...
SELECT 
    -- emp_nm
     NVL(emp_nm, '존재안함') AS emp_nm --이정직대신 박찬호 입력시 관련된 게 없으니 공집합을 null로 바꾸고 이후에 결과 없음을 조회함 4/26 ~11:07
FROM tb_emp
WHERE emp_nm = '이정직';
...
SELECT 
    emp_nm,
    NVL2(direct_manager_emp_no, '일반사원', '회장님') AS 직위 --DIRECT_MANAGER_EMP_NO 가 NULL 이면 회장님, 아니면 일반사원
FROM tb_emp;
