...
-- CROSS JOIN, 카테시안 곱(cartesian product) : 조건없이 모든 경우의 수를 다 매칭함
SELECT 
    *
FROM test_a, test_b -- join , 로 테이블 연결 가능, 가로로 합쳐진다
					-- 중첩 반복문의 형태는 곱 연산으로 이뤄짐
...
-- INNER JOIN (EQUI JOIN의 한 종류) , 조인 연결은 가로 연결 
SELECT 
    *
FROM test_a, test_b
...
SELECT
A.emp_no, B.certi_nm, A.CERTI_CD , B.CERTI_CD 
...
FROM tb_emp_certi A, tb_certi B, tb_emp C, tb_dept D --오라클조인
WHERE A.certi_cd = B.certi_cd
    AND A.emp_no = C.emp_no
...
SELECT 
    A.emp_no, A.emp_nm, A.addr, A.dept_cd, B.dept_nm
FROM tb_emp A 
inner join tb_dept B --,자리에 JOIN --ON 조건에 대한것만 같이 가져오는 걸 innerjoin이라고 함
 on A.dept_cd = B.dept_cd --JOIN 조건을 WHERE 절 앞에 ON 입력
WHERE A.addr LIKE '%용인%'   
    AND A.emp_nm LIKE '김%'
ORDER BY A.emp_no
;
...
SELECT 
    A.emp_no, A.emp_nm, dept_cd, B.dept_nm -- dept_CD는 앞에 테이블을 명시하면 안됨 왜?
FROM tb_emp A
NATURAL JOIN tb_dept B --중복되는 부분 하나만 나옴
;
